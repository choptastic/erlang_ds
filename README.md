...
## Modified Syntax Plugin (_experimental_/optional)
...
- `ds:update(Obj, ListOfKeys, Updater)` - Update the values in `Obj` associated
  ...
  it like this: `ds:update(Obj, ListOfKeys, fun(X) -> try list_to_integer(X) catch _:_ -> undefined end)`. Two default built-in updaters are `atomize`
...
- `boolize` - This will convert the values to a boolean. **Note:** This is
  highly accepting of various terms that might _mean_ false. For example, the
...
Worth noting is that `UpdaterArgs` must _always_ be one less than the
`FunctionArgs`.
...
- In _both_ expressions above `Obj` must be a variable. If `Obj` is anything
...
- A `string`, however, (despite being internally represented as a list of
  ...
  specifically tokenizes strings as their own thing. As a result: `Obj->"some string key"` is perfectly acceptable, and translates to `ds:get(Obj, "some string key")`.
...
- The inconsistencies of between the data structure modules makes me constantly
  ...
  `dict` as `find` and `store`, in `proplists` as `get_value` and _not
  implemented_ (basically, just delete the old value and prepend the new
...
Currently, the syntax-highlighting is dependent on [meck 0.9.2](https://github.com/eproxus/meck). The newest versions crash. This is being looked into.
